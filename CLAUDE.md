...
`<フィールド文字><数値><改行>` 形式の行を送ると、ステータスを直接設定できる。
...
`remoteSync()` が行バッファに溜めて解析し、`remoteDebugApply(pet)` が
...
- `fido_remote.html` — Web Serial API を使う単一 HTML ファイル。
  ...
  ヒント表示を現在の画面に追従させる。DEBUG セクションでステータスも設定可。
- シリアルモニタ (`pio device monitor`) で `a`/`b`/`c` や `h50` 等を直接打っても可。
