...
DEBUG セクションの入力欄、またはシリアルに直接打って送る。範囲外は自動クランプ。
...
| `s<n>` | health | 0-100 | `s0` |
| `g<n>` | age | 0-255 | `g80` |
| `m<n>` | coins | 0-9999 | `m999` |
| `k<n>` | bond (絆) | 0-1000 | `k800` |

適用時に `[DEBUG] x set to n` がシリアルに出る。

### アクションコマンド (語句 + 改行) — 新機能の即検証

待ち時間なしで昼夜・宇宙イベント・生死を検証できる。ACTION セクションのボタン
または語句を直接送る。

| コマンド | 動作 | 確認できること |
|----------|------|----------------|
| `night` | 夜に固定 | キャラ就寝表現(zZ) / 右上の三日月 / Talk・Play が夜挙動 |
| `sun` | 昼に固定 | 通常表示に戻る |
| `free` | 自動サイクルに復帰 | 約12分周期 (昼8分+夜4分) に戻る |
| `event` | 宇宙イベント即発生 | `[EVENT]` ログ / B5 チャイム / メッセージ表示 |
| `die` | 即離脱(死)→転生 | 追悼画面 → ボタンで転生 (コイン・アイテム継承) |

> ⚠ アクション語は a/b/c を含めない (含むとボタン押下に化ける)。`sun`/`free` 等。
...
`it0`-`it13`, `vol`, `nc`, `nc0`-`nc7`, `crit` (health0連続tick), `reb` (転生回数),
`bAge`/`bBond` (歴代最高記録)。
